 ...
 while(1)
 {
  ...
  switch(Choix)
  {
   ...
   case '2': ListingVehiculesHV ("VehiculesHV") ;
             break ;
   case '4': printf("Saisir la reference : ");
             fgets(Tampon, sizeof(Tampon), stdin);
             Numero = atoi(Tampon);
             res = RechercheMV("VehiculesHV", Numero, &UnRecord);

             if(res == 1) // si la recherche a bien été effectuée..
             {
                AfficheEnteteVehiculeHV();
                AfficheVehiculeHV(&UnRecord);
             }
             if(res == 0) printf("La recherche n'a pas su trouver la reference demandée.\n");

             break ;
   case '6': ListingFacturesHV("FactureHVsz") ;
    break;
    case '5' : 

            printf("Nom du client : ");
            fgets(Tampon, sizeof(Tampon), stdin);
            strcpy(NomClient, Tampon);
            ...
            if(res == 1) // si la recherche a bien été effectuée..
             {
                ...
                achat = ReservationHV("VehiculesHV", Numero, Quantite, &UnRecord);

                if(achat == 1)
                {
                  printf("Mise à jour du fichier faite !\n");
                  time_t currentTime;
                  fct = FacturationHV("FactureHV", NomClient, time(&currentTime), Quantite, Numero);
	     ...
	     break ;
   case '7': AProposServeurHV("V 1","Vanderheyden","Mickael") ;
             break ;
